...
<body class="min-h-screen bg-slate-100 text-slate-900 dark:bg-slate-950 dark:text-slate-100" x-data="appShell">
    <div x-cloak x-show="sidebarOpen" @click="sidebarOpen = false" class="fixed inset-0 z-40 bg-slate-950/60 lg:hidden"></div>

    <aside class="fixed inset-y-0 left-0 z-50 flex w-72 -translate-x-full flex-col border-r border-slate-800 bg-slate-950 text-slate-100 transition-transform duration-200 lg:translate-x-0"
        ...
        <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-6">
            @foreach ($links as [$routeName, $label, $hint])
                ...
                <a href="{{ route($routeName) }}"
                   class="flex items-center justify-between rounded-xl px-4 py-3 transition {{ $isActive ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-950/30' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }}">
        ...
        <div class="border-t border-slate-800 p-4">
            <a href="{{ route('home') }}" class="block rounded-xl px-4 py-3 text-sm font-semibold text-slate-300 hover:bg-slate-900 hover:text-white">View public catalog</a>
        </div>
    </aside>

    <div class="min-h-screen lg:pl-72">
        <header class="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur sm:px-6 dark:border-slate-800 dark:bg-slate-900/90">
            <div class="flex items-center gap-3">
                <button @click="sidebarOpen = true" class="grid size-10 place-items-center rounded-xl border border-slate-200 lg:hidden dark:border-slate-700" aria-label="Open navigation">Menu</button>
                <div>
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                <button type="button" @click="toggleTheme()" class="grid size-10 place-items-center rounded-xl border border-slate-200 dark:border-slate-700" aria-label="Toggle color theme">
