...
@section('content')
<div class="mb-5 flex justify-end"><a href="{{ route('admin.books.create') }}" class="btn-primary">Add book</a></div>
...
@else
    <div class="panel overflow-hidden"><div class="overflow-x-auto"><table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
        ...
        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">@foreach ($books as $book)<tr>
            ...
            <td class="px-5 py-4 text-right"><a class="font-semibold text-indigo-600" href="{{ route('admin.books.edit', $book) }}">Edit</a><button @click="$dispatch('open-modal', 'delete-book-{{ $book->id }}')" class="ml-3 font-semibold text-red-600">Archive</button><x-modal name="delete-book-{{ $book->id }}" title="Archive book?"><p>The record will be soft deleted. Active loans prevent this action.</p><x-slot:actions><form method="POST" action="{{ route('admin.books.destroy', $book) }}">@csrf @method('DELETE')<button class="btn-danger">Archive</button></form></x-slot:actions></x-modal></td>
        </tr>@endforeach</tbody>
    </table></div></div>
