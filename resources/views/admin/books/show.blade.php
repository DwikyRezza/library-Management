...
<div class="grid gap-6 xl:grid-cols-[1fr_360px]">
        ...
        <section class="panel p-6">
            <div class="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
                <div class="min-w-0">
                    ...
                    <h2 class="mt-3 text-2xl font-black text-slate-950 dark:text-white">{{ $book->title }}</h2>
                    <p class="mt-1 text-slate-500 dark:text-slate-400">{{ $book->author }}</p>
                </div>
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('admin.books.edit', $book) }}" class="btn-secondary">Edit metadata</a>
                    <button type="button" @click="$dispatch('open-modal', 'delete-book-detail')" class="btn-danger">Hapus buku</button>
                </div>
            </div>

            <dl class="mt-6 grid gap-4 border-t border-slate-200 pt-5 text-sm sm:grid-cols-3 dark:border-white/10">
                <div>
                    <dt class="text-slate-500 dark:text-slate-400">Category</dt>
                    <dd class="mt-1 font-semibold">{{ $book->category->name }}</dd>
                </div>
                <div>
                    <dt class="text-slate-500 dark:text-slate-400">Publisher / year</dt>
                    <dd class="mt-1 font-semibold">{{ $book->publisher ?: 'N/a' }} - {{ $book->publication_year ?: 'N/a' }}</dd>
                </div>
                <div>
                    <dt class="text-slate-500 dark:text-slate-400">ISBN</dt>
                    <dd class="mt-1 font-semibold">{{ $book->isbn ?: 'N/a' }}</dd>
                </div>
            </dl>

            @if ($book->description)
                <p class="mt-5 text-sm leading-7 text-slate-600 dark:text-slate-300">{{ $book->description }}</p>
            @endif
        </section>

        <section class="panel overflow-hidden">
            <div class="border-b border-slate-200 p-5 dark:border-white/10">
                <h2 class="font-bold">Physical copies ({{ $book->total_copies }})</h2>
            </div>
                ...
                <table class="soft-table">
                    ...
                    <tbody>
                        @foreach ($book->copies as $copy)
                            <tr>
                                <td class="font-mono text-xs font-bold">{{ $copy->copy_code }}</td>
                                <td>{{ $copy->shelf_location ?: 'N/a' }}</td>
                                <td><x-badge :status="$copy->status" /></td>
                                <td class="text-slate-500 dark:text-slate-400">{{ $copy->condition_note ?: 'No note' }}</td>
                                <td class="text-right">
                                    @if ($copy->status !== \App\Models\BookCopy::STATUS_BORROWED)
                                        <button type="button" @click="$dispatch('open-modal', 'copy-{{ $copy->id }}')" class="subtle-link">Change</button>
                                        <x-modal name="copy-{{ $copy->id }}" title="Update {{ $copy->copy_code }}">
                                            <form id="copy-form-{{ $copy->id }}" method="POST" action="{{ route('admin.book-copies.update', $copy) }}" class="space-y-4">
                                                @csrf
                                                @method('PATCH')
                                                <div>
                                                    <label class="label">Status</label>
                                                    <select class="input" name="status">
                                                        @foreach (['available', 'maintenance', 'lost'] as $status)
                                                ...
                                                <div>
                                                    ...
                                                    <textarea class="input" name="condition_note">{{ $copy->condition_note }}</textarea>
                                                </div>
                                            </form>
                                            <x-slot:actions>
                                                <button form="copy-form-{{ $copy->id }}" class="btn-primary">Save</button>
                                            </x-slot:actions>
                                        </x-modal>
                                    @else
            ...
            <div class="divide-y divide-slate-100 dark:divide-white/10">
                @forelse ($recentTransactions as $transaction)
                    <a href="{{ route('admin.transactions.show', $transaction) }}" class="flex items-center justify-between gap-4 p-5 transition hover:bg-slate-50 dark:hover:bg-white/5">
                        <div class="min-w-0">
                            <p class="truncate font-semibold">{{ $transaction->member->full_name }}</p>
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ $transaction->transaction_code }} - {{ $transaction->bookCopy->copy_code }}</p>
                        </div>
                        <x-badge :status="$transaction->display_status" />
                    </a>
                @empty
                    <p class="p-5 text-sm text-slate-500 dark:text-slate-400">No transactions for this book.</p>
                @endforelse
            </div>
        </section>
    </div>
    ...
    <aside class="space-y-6">
        <form method="POST" action="{{ route('admin.books.copies.store', $book) }}" class="panel p-5">
            @csrf
            <h2 class="font-bold">Add physical copies</h2>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Existing copies remain unchanged.</p>
            <div class="mt-5">
                <label class="label">Number of copies</label>
                <input class="input" type="number" min="1" max="200" name="number_of_copies" value="1" required>
            </div>
            <div class="mt-4">
                <label class="label">Shelf location</label>
                <input class="input" name="shelf_location" placeholder="Example: A-12">
            </div>
            <button class="btn-primary mt-5 w-full">Generate copies</button>
        </form>
                ...
                @if ($book->digitalAsset)
                    <div class="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm dark:border-white/10 dark:bg-white/5">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span class="text-xs text-slate-500 dark:text-slate-400">{{ number_format($book->digitalAsset->file_size / 1024, 1) }} KB</span>
                        </div>
